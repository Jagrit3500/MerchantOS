...
ESCALATION_TIERS = [
    ...
    },
    {
        ...
        "rbi_ref": config.OMBUDSMAN_SCHEME_REFERENCE,
    },
    {
        "tier": 5,
        ...
        "rbi_ref": f"{config.CONSUMER_LAW_REFERENCE} - eligibility and commercial-purpose exclusions require review",
    },
]
...
ISSUE_RBI_REFS = {
    ...
    "fee_overcharge":     f"Merchant agreement and published pricing; {config.PA_DIRECTIONS_REFERENCE}, {config.PA_PRICING_REFERENCE}",
...
class EscalationAgent:
    ...
    def get_all_tiers(self) -> list[dict]:
        return [dict(tier) for tier in ESCALATION_TIERS]
    ...
    def draft_grievance_letter(self, merchant: dict, issue: dict) -> str:
        ...
        lines += [
            ...
            "",
            "MY REQUEST:",
            "  1. Immediate resolution of the above issue",
            "  2. Written explanation citing the applicable agreement, policy, or regulatory basis",
            f"  3. The expected review or release date for any held funds ({INR}{issue.get('amount', 'N/A')})",
            "  4. Confirmation of actions taken via email",
            "",
            "If I receive no satisfactory response, I will review whether this complaint is eligible under",
    ...
    def draft_rbi_ombudsman_complaint(self, merchant: dict, issue: dict) -> str:
        ...
        lines = [
            "COMPLAINT TO RBI INTEGRATED OMBUDSMAN",
            ...
            "",
            "SECTION B - PAYMENT AGGREGATOR DETAILS",
            f"  Name:    {_agg_name}",
            ...
            f"  {timing_statement}",
            "",
            "SECTION E - POTENTIALLY RELEVANT PROVISIONS",
            f"  1. {rbi_ref}",
            ...
            f"  2. Written explanation from {_agg_short} citing the applicable agreement, policy, or regulatory basis",
            "  3. Any other relief the Ombudsman considers permissible, if this complaint is maintainable",
            "",
            "SECTION G - GRIEVANCE HISTORY",
        ...
        lines += [
            "  Filing note: Confirm that the applicable complaint prerequisites and waiting period are satisfied.",
            "",
            "DECLARATION:",
    ...
    def draft_legal_notice(self, merchant: dict, issue: dict) -> str:
        ...
        lines = [
            ...
            "",
            "4. Subject to review by a qualified advocate, the facts may raise issues under:",
            "   a) The applicable service agreement and provider policies",
            f"   b) {config.CONSUMER_LAW_REFERENCE}, if the complainant and transaction qualify",
            ...
            "   2. Address any documented loss or compensation claim supported by evidence and legal advice.",
            "",
            "If unresolved, I reserve any remedies available under the agreement and applicable law,",
            "including a consumer complaint only if counsel confirms eligibility and jurisdiction.",
            "",
            f"{merchant['name']}",
